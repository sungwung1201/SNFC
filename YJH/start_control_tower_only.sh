...
echo -e "${BLUE}   (주의: 가상 AMR 및 일반 적재(SG2 In) 노드는 실행하지 않습니다.)  ${NC}"
echo -e "${BLUE}================================================================${NC}"
...
case $RUN_MODE in
    1)
        if command -v gnome-terminal >/dev/null 2>&1; then
            ...
            gnome-terminal --tab --title="ROS2 Mock SG2 Out" -- bash -c "source install/setup.bash && export ROS_DOMAIN_ID=119 && export ROS_LOCALHOST_ONLY=$ROS_LOCALHOST_ONLY && ros2 run cobot3 mock_sg2_out; exec bash"
            ...
            sleep 0.5
            x-terminal-emulator -e bash -c "source install/setup.bash && export ROS_DOMAIN_ID=119 && export ROS_LOCALHOST_ONLY=$ROS_LOCALHOST_ONLY && ros2 run cobot3 mock_sg2_out" &
        else
